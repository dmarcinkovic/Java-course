...
public class CatShellCommand extends ShellCommandUtil implements ShellCommand {
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		if (allArguments.length == 1) {
			fileContent = getFileContent(allArguments[0], env);
		} else {
			fileContent = getFileContent(allArguments[0], allArguments[1], env);
		}
	 ...
	 */
	private List<String> getFileContent(String filename, String charsetName, Environment env) {
		Charset c = null;
		try {
	 ...
	 * @return List of strings representing the content of the file.
	 */
	private List<String> getFileContent(String filename, Environment env) {
		return getFileContent(filename, "UTF-8", env);
	}
